...
def merge(Og_and_copy: dict[str, list[int]], parent_sub: dict, l_sub: dict, r_sub: dict, og_is_ref: bool) -> tuple[dict[str, list[int]], bool]:
    if((parent_sub["end"]-parent_sub["start"]) > 1):
        og_is_ref = not og_is_ref
    l_index = l_sub["start"]
    r_index = r_sub["start"]
    ref = "Og" if og_is_ref else "copy"
    working = "copy" if og_is_ref else "Og"
    ...
    while (r_index <= r_sub["end"]):
        ...
        r_index += 1
        parent_index += 1
    # if (r_sub["end"] == len(Og_and_copy[ref])-1):
    #     og_is_ref = not og_is_ref
    print(Og_and_copy, og_is_ref)
    return Og_and_copy, og_is_ref


def merge_Sort(Og_and_copy: dict[str, list[int]], og_is_ref: bool, array_info: dict[str, int]) -> tuple[dict[str, list[int]], bool]:
    if ((len(Og_and_copy["Og"]) < 2) or ((array_info["end"] - array_info["start"] + 1) < 2)):
        return Og_and_copy, og_is_ref
    l_sub = {"start": array_info["start"], "end": math.floor(
        (array_info["start"] + array_info["end"])/2)}
    r_sub = {"start": l_sub["end"]+1, "end": array_info["end"]}
    Og_and_copy, og_is_ref= merge_Sort(Og_and_copy, og_is_ref, l_sub)
    Og_and_copy, og_is_ref = merge_Sort(Og_and_copy, og_is_ref, r_sub)
    Og_and_copy, og_is_ref = merge(Og_and_copy, array_info, l_sub, r_sub, og_is_ref)
    return Og_and_copy, og_is_ref
...
def Optimized_Merge_sort(Og_array: list[int]) -> list[int]:
    ...
    Og_and_copy = {"Og": Og_array, "copy": copy_array}
    og_is_ref = True
    array_info = {"start": 0, "end": len(Og_and_copy["Og"])-1}
