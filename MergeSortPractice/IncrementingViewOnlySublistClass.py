...
def merge(Original_and_copy: dict[str, list[int]], parent_sub: SubList, l_sub: SubList, r_sub: SubList, og_is_ref: bool) -> None:
    l_index = l_sub.start
    r_index = r_sub.start
    ...
    while (l_index <= l_sub.end and r_index <= r_sub.end):
        if (Original_and_copy[ref][l_index] <= Original_and_copy[ref][r_index]):
            Original_and_copy[working][parent_index] = Original_and_copy[ref][l_index]
            l_index += 1
        else:
            Original_and_copy[working][parent_index] = Original_and_copy[ref][r_index]
            r_index += 1
        parent_index += 1
...
def Optimized_Merge_sort(Oiriginal_array: list[int]) -> list[int]:
    copy_array = []
    for num in Oiriginal_array:
        copy_array.append(num)
    Original_and_copy = {"Original": Oiriginal_array, "copy": copy_array}
    origianl_is_ref = True
    array_info = SubList(0, len(Original_and_copy["Original"])-1)
    merge_Sort(Original_and_copy, origianl_is_ref, array_info)
    if (not origianl_is_ref): 
        Original_and_copy["Original"]= Original_and_copy["copy"][:]
    return Original_and_copy["Original"]
